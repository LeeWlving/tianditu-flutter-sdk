...
class AdministrativeResult {
  ...
  AdministrativeResult({this.status, this.message, this.data});
...
class DataDTO {
  ...
  /// 创建DataDTO实例
  DataDTO({this.suggestion, this.district});
  ...
  /// 转换为JSON
  Map<String, dynamic> toJson() {
    return {'suggestion': suggestion, 'district': district?.toJson()};
  }
}
...
class CenterDTO {
  ...
  /// 创建CenterDTO实例
  CenterDTO({this.lng, this.lat});

  /// 从JSON字符串解析CenterDTO
  factory CenterDTO.fromJson(Map<String, dynamic> json) {
    return CenterDTO(lng: json['lng'] as double?, lat: json['lat'] as double?);
  }
